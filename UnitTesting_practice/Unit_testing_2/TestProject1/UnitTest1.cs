...
public class CalculatorTests
{
    ...
    [Test]
    public void GetVal_AllCases_InSingleTest()
    {
        MultiExcept obj = new MultiExcept();

        Assert.Multiple(() =>
        {
            var ex1 = Assert.Throws<DivideByZeroException>(() => obj.getval(0));
            Assert.AreEqual("Value should be more than zero", ex1.Message);

            var ex2 = Assert.Throws<Exception>(() => obj.getval(1));
            Assert.AreEqual("val should more than one", ex2.Message);

            Assert.AreEqual(5, obj.getval(5));
        });
    }
}
